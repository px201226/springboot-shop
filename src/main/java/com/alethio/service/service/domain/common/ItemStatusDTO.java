...
public class ItemStatusDTO {
    ...
    private Long requestStockThreshold;                 // 재고 부족 임계값
    private Long requestStockQuantity;                  // 입고 요청 시 입고 요청하는 재고 수
    private String encryptKey;                          // 밴더사를 위한 암호화 키
    private Boolean isStockQuantityLessThreshold;       // 현재 재고가 임계값보다 미만인지
    ...
    public static ItemStatusDTO of(ItemEntity item){
        return ItemStatusDTO.builder()
                ...
                .isStockQuantityLessThreshold(item.isAvailableStockLessThreshold())
                .build();
    }
}
